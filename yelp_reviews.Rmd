...
get_restaurants_with_diff_ratings = function(ethnicity, n_restaurants, eth_names) {
  ...
  p_value_cutoff = 0.05 / n_restaurants
...
see if they exist in any first names. This runs in quadratic time so it is fairly
slow.
...
```{r}
korean_names = read.csv("top_100_baby_names_korean.csv", stringsAsFactors = FALSE)
korean_names$name = tolower(korean_names$name)

japanese_names = read.csv("top_100_baby_names_japanese.csv", stringsAsFactors = FALSE)
japanese_names$name = tolower(japanese_names$name)

korean_users = semi_join(users, korean_names)
japanese_users = semi_join(users, japanese_names)

east_asian_names = as.data.frame(c(chinese_names2$name, japanese_names$name, korean_names$name), stringsAsFactors = FALSE)
names(east_asian_names) = c("name")
east_asian_users = semi_join(users, east_asian_names)

results = get_restaurants_with_diff_ratings('Chinese|Korean|Japanese', 100, east_asian_names)
results
```

# East Asian Users using pinyin for Chinese names
```{r east asian restaurants}
east_asian_names = as.data.frame(c(chinese_names$name, japanese_names$name, korean_names$name), stringsAsFactors = FALSE)
names(east_asian_names) = c("name")
east_asian_users = semi_join(users, east_asian_names)
