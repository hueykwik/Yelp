...
```{r}
n = 50
summaries = ReviewCountsForEthnicity(latino.names)
n.eth.summaries = head(summaries[[1]], n)
select(n.eth.summaries, name, num.reviews)
```

Top 50 restaurants among non-Latinos, based on number of reviews
```{r}
n.non.eth.summaries = head(summaries[[2]], n)
select(n.non.eth.summaries, name, num.reviews)
```

Comparing the two sets, which restaurants are in the Top 50 for both groups?
```{r}
n.eth.summaries = select(n.eth.summaries, business_id, name)
n.non.eth.summaries = select(n.non.eth.summaries, business_id, name)

intersect(n.eth.summaries, n.non.eth.summaries) %>% select(name)
```
Surprisingly, these two groups share 38/50 (74%) of the same restaurants.

Restaurants that are popular among Latinos but not among non-Latinos:
```{r}
setdiff(n.eth.summaries[1], n.non.eth.summaries[1])
```

Vice-versa:
```{r}
setdiff(n.non.eth.summaries[1], n.eth.summaries[1])
```

Similar analysis but for East Asians.
Top 50 restaurants among East Asians, based on number of reviews:
```{r}
summaries = ReviewCountsForEthnicity(east.asian.names)
n.eth.summaries = head(summaries[[1]], n)
n.eth.summaries
```
...
```{r category utility}
get_categories_dict = function(categories) {
...
Comparing the two sets, which restaurants are in the Top 50 for both groups?
```{r}
intersect(n.eth.summaries[1], n.non.eth.summaries[1])
```
26/50 are shared between the sets. This is more different than the comparison 
we made with Latinos above.
...
setdiff(n.non.eth.summaries[1], n.eth.summaries[1])
```
It seems like common American chain restaurants are in this list, e.g. Subway,
Denny's, Chipotle, P.F. Chang's.
